  ...
  return (
    <div className="min-h-screen bg-[#121212] text-white font-sans selection:bg-red-600 selection:text-white relative pb-24">
      ...
      <main className="relative z-10 max-w-4xl mx-auto px-6 pt-32">
        <div className="mb-12">
           <h1 className="text-4xl font-black uppercase tracking-tighter text-white mb-2">Edit Listing</h1>
        ...
        <div className="space-y-16">
           ...
           <section className="space-y-8">
              <div className="flex items-center justify-between border-b border-white/10 pb-4">
                 <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-xl font-bold uppercase tracking-widest text-white">Visuals</h2>
                 </div>
              ...
              <div className="space-y-4">
                 ...
                 <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {existingImages.map((img, i) => (
                       <div key={i} className="relative group aspect-square bg-[#1a1a1a] rounded-xl border border-white/10 overflow-hidden">
                          <img src={img} className="w-full h-full object-cover" />
                          <button onClick={() => removeExistingImage(i)} className="absolute top-2 right-2 bg-black/80 text-white p-1.5 rounded-full hover:bg-red-600 transition-colors opacity-0 group-hover:opacity-100">
                             <Trash2 className="w-3 h-3" />
                          </button>
                          <div className="absolute bottom-2 left-2 text-[10px] font-mono font-bold text-white/50 bg-black/50 px-1 rounded">{i === 0 ? "COVER" : `IMG_${i+1}`}</div>
                       </div>
                    ))}
                 </div>
              </div>
              ...
              <div className="space-y-4">
                 ...
                 <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {newImages.map((file, i) => (
                       <div key={i} className="relative group aspect-square bg-[#1a1a1a] rounded-xl border border-green-500/30 overflow-hidden">
                          <img src={URL.createObjectURL(file)} className="w-full h-full object-cover" />
                          <button onClick={() => removeNewImage(i)} className="absolute top-2 right-2 bg-black/80 text-white p-1.5 rounded-full hover:bg-red-600 transition-colors">
                             <Trash2 className="w-3 h-3" />
                          </button>
                          <div className="absolute bottom-2 left-2 text-[10px] font-mono font-bold text-green-400 bg-black/50 px-1 rounded">NEW</div>
                       </div>
                    ))}
                    
                    {/* Add Button */}
                    <label className="aspect-square border border-dashed border-white/10 rounded-xl flex flex-col items-center justify-center hover:border-white/30 hover:bg-white/5 transition-all cursor-pointer">
                       <Plus className="w-6 h-6 text-white/20 mb-2" />
                       <span className="text-[8px] font-bold uppercase tracking-widest text-white/40">Add More</span>
           ...
           <section className="space-y-8">
              <div className="flex items-center gap-4 border-b border-white/10 pb-4">
                 ...
                 <h2 className="text-xl font-bold uppercase tracking-widest text-white">Data Points</h2>
              </div>
              ...
              <div className="space-y-6">
                 <div className="space-y-2 group">
                    ...
                    <input type="text" value={title} onChange={e => setTitle(e.target.value)} className="w-full bg-[#1a1a1a] border border-white/10 rounded-xl px-4 py-3 text-white focus:border-red-600 focus:outline-none transition-all font-mono text-sm" />
                 </div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2 group">
                       ...
                       <input type="number" value={price} onChange={e => setPrice(e.target.value)} className="w-full bg-[#1a1a1a] border border-white/10 rounded-xl px-4 py-3 text-white focus:border-red-600 focus:outline-none transition-all font-mono text-sm" />
                    </div>
                    <div className="space-y-2 group">
                       ...
                       <input type="text" value={location} onChange={e => setLocation(e.target.value)} className="w-full bg-[#1a1a1a] border border-white/10 rounded-xl px-4 py-3 text-white focus:border-red-600 focus:outline-none transition-all font-mono text-sm" />
                    </div>
                 </div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2 group">
                       ...
                       <input type="text" value={category} onChange={e => setCategory(e.target.value)} className="w-full bg-[#1a1a1a] border border-white/10 rounded-xl px-4 py-3 text-white focus:border-red-600 focus:outline-none transition-all font-mono text-sm" />
                    </div>
                    <div className="space-y-2 group">
                       ...
                       <select value={condition} onChange={e => setCondition(e.target.value)} className="w-full bg-[#1a1a1a] border border-white/10 rounded-xl px-4 py-3 text-white focus:border-red-600 focus:outline-none transition-all font-mono text-sm appearance-none cursor-pointer">
                 ...
                 <div className="space-y-2 group">
                    ...
                    <textarea value={description} onChange={e => setDescription(e.target.value)} rows={5} className="w-full bg-[#1a1a1a] border border-white/10 rounded-xl px-4 py-3 text-white focus:border-red-600 focus:outline-none transition-all font-mono text-sm resize-none" />
                 </div>
              </div>
           </section>

           {/* Submit */}
           <button onClick={handleUpdate} disabled={updating} className="w-full py-5 bg-red-600 text-white rounded-xl text-sm font-black uppercase tracking-widest hover:bg-red-700 transition-all shadow-[0_0_30px_rgba(220,38,38,0.4)] flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed group">
              {updating ? (
                 <><Loader2 className="w-5 h-5 animate-spin" /> Patching Database...</>
              ) : (
                 <><Save className="w-5 h-5 group-hover:scale-110 transition-transform" /> Save Updates</>
              )}
           </button>
